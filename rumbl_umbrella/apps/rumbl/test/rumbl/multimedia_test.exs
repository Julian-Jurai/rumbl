defmodule Rumbl.MultimediaTest do 
  ...
  describe "categories" do 
    test "list_alphabetical_categories/0" do 
      ...
      alpah_names = for %Category{name: name} <- Multimedia.list_alphabetical_categories() do 
        name
      end

      assert alpah_names == ~w(Action Comedy Drama)
    end
  end
  ...
  describe "videos" do 
   ...
   @invalid_attrs %{ description: nil, title: nil, url: nil } 
    ...
    test "create_video/2 with invalid data returns and error changeset" do 
      owner = user_fixture()
    ...
    test "update_video/2 with invalid data returns and error changeset" do 
      owner = user_fixture()
